...
class ProcessRunner {
    logger;
    constructor(runId) {
        this.logger = new logger_1.Logger(runId);
    }
    static async startProcess(processName, projectId, inputs) {
        ...
        const def = yaml.load(fs.readFileSync(processPath, 'utf8'));
        const processRunId = await new Promise((resolve, reject) => {
            ...
            stmt.run(processName, projectId, JSON.stringify(inputs), function (err) {
                if (err)
                    reject(err);
                else
                    resolve(this.lastID);
        ...
        for (const step of def.steps) {
            await new Promise((resolve, reject) => {
                ...
                stmt.run(processRunId, step.id, (err) => {
                    if (err)
                        reject(err);
                    else
                        resolve(true);
    ...
    async executeNextStep(processRunId) {
        const db = db_1.DB.getInstance().getDb();
        const step = await new Promise((resolve) => {
            db.get("SELECT * FROM process_steps WHERE process_run_id = ? AND status = 'PENDING' ORDER BY id ASC LIMIT 1", [processRunId], (err, row) => resolve(row));
        });
        if (!step) {
        ...
        const def = yaml.load(fs.readFileSync(processPath, 'utf8'));
        const stepDef = def.steps.find((s) => s.id === step.step_id);
        ...
        try {
            ...
            const prevSteps = await new Promise((resolve) => {
            ...
            if (stepDef.action_type === 'LLM_TASK') {
                ...
                if (stepDef.quality_gates) {
                    const { QualityGate } = await Promise.resolve().then(() => __importStar(require('./quality_gate')));
...
`;
                ...
                const { WisdomLayer } = await Promise.resolve().then(() => __importStar(require('./wisdom_layer')));
                ...
                const { Metronome } = await Promise.resolve().then(() => __importStar(require('./metronome')));
                ...
                const { Multiplexer } = await Promise.resolve().then(() => __importStar(require('./multiplex')));
                ...
                const { Polisher } = await Promise.resolve().then(() => __importStar(require('./polisher')));
                ...
                const { UniversalProxy } = await Promise.resolve().then(() => __importStar(require('./universal_proxy')));
...
`;
                ...
                const { PerformanceReview } = await Promise.resolve().then(() => __importStar(require('./performance')));
                ...
                const { Scout } = await Promise.resolve().then(() => __importStar(require('./scout')));
...
`;
                ...
                const { DataIngestor } = await Promise.resolve().then(() => __importStar(require('./ingestion')));
    ...
    async finalizeProcess(processRunId) {
        const db = db_1.DB.getInstance().getDb();
        const processDir = path.join(process.cwd(), 'runs', `process_${processRunId}`);
