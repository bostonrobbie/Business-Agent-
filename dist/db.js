...
class DB {
    ...
    db;
    constructor() {
        this.db = new sqlite3_1.default.Database('company_os.db');
    }
    static getInstance() {
        if (!DB.instance) {
            DB.instance = new DB();
            ...
            this.db.serialize(() => {
                ...
                this.db.run(logSchema, (err) => {
                    if (err)
                        ...
                        this.db.all("PRAGMA table_info(runs)", (err, rows) => {
                            ...
                            this.db.all("PRAGMA table_info(memories)", (err, mRows) => {
                            ...
                            this.db.get("SELECT count(*) as c FROM projects", (err, row) => {
                                if (row.c === 0) {
